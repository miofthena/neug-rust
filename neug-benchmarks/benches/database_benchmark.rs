...
fn bench_graph_operations(c: &mut Criterion) {
    ...
    conn.execute("CREATE NODE TABLE Person(id INT64, name STRING, PRIMARY KEY(id));").unwrap();
    conn.execute("CREATE REL TABLE FOLLOWS(FROM Person TO Person, weight DOUBLE);").unwrap();
    ...
    c.bench_function("graph_insert_node", |b| {
        b.iter_batched(
            || {
                ...
                format!("CREATE (:Person {{id: {}, name: 'User{}'}});", current_id, current_id)
            },
            |query| {
    ...
    for i in 0..100 {
        let _ = conn.execute(&format!("CREATE (:Person {{id: {}, name: 'Populated{}'}});", 1000000 + i, i));
        if i > 0 {
...
criterion_group!(benches, bench_connection_lifecycle, bench_query_dispatch, bench_graph_operations);
