use neug_rust::{Database, Mode, DatabaseOptions};
...
fn test_parallel_query_executions() {
    ...
    for i in 0..10 {
        ...
        let t = thread::spawn(move || {
            for j in 0..10 {
                let node_id = i * 10 + j;
                let query = format!("CREATE (p: person {{id: {}, name: 'Node{}'}});", node_id, node_id);
...
fn test_access_mode() {
    ...
    for mode in modes {
        let query = format!("CREATE NODE TABLE test_table_{}(id INT64, PRIMARY KEY(id));", mode.as_str());
